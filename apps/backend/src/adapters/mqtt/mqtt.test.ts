...
vi.mock('fs', () => ({
  readFileSync: vi.fn().mockImplementation(path => Buffer.from(`mock content for ${path}`)),
}));
...
describe('Mqtt', () => {
  ...
  describe('addListener', () => {
    it('should add a listener to the listeners array', async () => {
      // Arrange
      const listener = {
        subscribeTopic: vi.fn().mockReturnValue(['test/topic']),
        onMQTTMessage: vi.fn(),
      };
    ...
    it('should not notify listeners for unsubscribed topics', async () => {
      // Arrange
      const listener = {
        subscribeTopic: vi.fn().mockReturnValue(['subscribed/topic']),
        onMQTTMessage: vi.fn(),
      };
      mqttClient.addListener(listener);
