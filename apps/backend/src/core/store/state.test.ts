...
describe('StateStore', () => {
  ...
  describe('start', () => {
    it('should load state and set up save interval', () => {
      // Arrange
      vi.useFakeTimers();
      const loadSpy = vi.spyOn(stateStore, 'load');
      const saveSpy = vi.spyOn(stateStore as any, 'save');

      // Act
      stateStore.start();
      vi.advanceTimersByTime(1000 * 60 * 2); // Advance by 2 minutes
  ...
  describe('stop', () => {
    it('should clear timer and save state', () => {
      ...
      // Assert
      expect(saveSpy).toHaveBeenCalledTimes(1);
      vi.advanceTimersByTime(1000 * 60 * 10); // Advance by 10 minutes
  ...
  describe('load', () => {
    it('should load state from file if it exists', () => {
      ...
      (fs.readFileSync as any).mockReturnValue(JSON.stringify(mockState));
...
describe('DeviceStore', () => {
  ...
  describe('start', () => {
    it('should load devices and set up save interval', () => {
      // Arrange
      vi.useFakeTimers();
      const loadSpy = vi.spyOn(deviceStore, 'load');
      const saveSpy = vi.spyOn(deviceStore as any, 'save');
  ...
  describe('load', () => {
    it('should load devices from file if it exists', () => {
      ...
      (fs.existsSync as any).mockReturnValue(true);
      (fs.readFileSync as any).mockReturnValue(JSON.stringify(mockDevices));
