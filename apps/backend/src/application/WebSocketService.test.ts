...
vi.mock('socket.io', () => {
  ...
  const mockOf = vi.fn().mockReturnValue(mockNamespace);
  const mockServer = {
    of: mockOf,
  };
...
vi.mock('../utils/logger', () => {
  return {
    ...
    LogEventTransport: vi.fn(),
  };
});

vi.mock('../utils/utils', () => ({
  ProxyConfig: {
    getSocketNamespace: vi.fn().mockReturnValue('/test-namespace'),
  },
}));
...
describe('WebSocketService', () => {
  ...
  const initServiceAndGetMocks = () => {
    ...
    mockServer = (SocketIOServer as unknown as any).mock.results[0].value;
  ...
  describe('getAllLogs', () => {
    it('should send all stored messages', () => {
      ...
      const sendLogSpy = vi.spyOn(webSocketService, 'sendLog');
