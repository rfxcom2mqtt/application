...
function JournalsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setConnectionError(
        `Failed to initialize WebSocket connection: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
      setConnecting(false);
    }
  }, []);
  ...
  const connectionStatusActions = (
    <Chip
      ...
      label={
        connecting
          ? t('journals.connection.connecting')
          : socket
            ? t('journals.connection.connected')
            : t('journals.connection.disconnected')
      }
