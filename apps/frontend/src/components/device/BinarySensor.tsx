      ...
      <CardContent sx={{ p: 2 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', flexGrow: 1, mr: 1 }}>
            <Box
              sx={{ mr: 1.5, color: currentValue ? `${statusInfo.color}.main` : 'text.disabled' }}
            >
        ...
        <Box sx={{ pt: 1 }}>
          ...
          >
            <Typography variant="caption">ON: {item.value_on.toString()}</Typography>
            <Typography variant="caption">OFF: {item.value_off.toString()}</Typography>
          </Stack>
        </Box>
      </CardContent>
    </Card>
