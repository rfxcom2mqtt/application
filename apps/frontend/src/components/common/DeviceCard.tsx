...
function DeviceCard({ device }: DeviceCardProps) {
  ...
  const handleQuickAction = (_action: string) => {
    ...
    // Quick action implementation will be added here
  };
      ...
      <CardContent sx={{ flexGrow: 1, pb: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Tooltip
              title={t('deviceCard.deviceStatus', {
                status: t(`devices.status.${device.status || 'unknown'}`),
              })}
            >
        ...
        <Stack spacing={1.5}>
          ...
          <DeviceInfoRow
            ...
            value={
              <Chip label={device.entityCount || 0} size="small" color="primary" variant="filled" />
            }
          />
        </Stack>
...
function DeviceInfoRow({ label, value }: DeviceInfoRowProps) {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {typeof value === 'string' ? <Chip label={value} size="small" variant="outlined" /> : value}
    </Box>
  );
}
