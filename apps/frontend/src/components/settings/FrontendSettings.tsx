...
import { Box, Typography, Divider, Paper, Grid, Collapse, Alert } from '@mui/material';
import {
  Web as WebIcon,
      ...
      >
        ...
        {!settings.enabled && (
          <Alert severity="info" sx={{ mt: 2 }}>
            The web interface is currently disabled. Enable it to access the configuration and
      ...
      <Collapse in={settings.enabled}>
          ...
          {settings.host && !settings.host.startsWith('/') && (
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>Access URL:</strong> http://{settings.host || 'localhost'}:
                {settings.port || 8080}
              </Typography>
            </Alert>
          )}
        ...
        >
          <Alert severity="warning" sx={{ mb: 3 }}>
            <Typography variant="body2">
              <strong>Important:</strong> Both certificate and key files must be provided to enable
              HTTPS. Leave both fields empty to use HTTP.
            </Typography>
          </Alert>
          ...
          {settings.sslCert &&
            settings.sslKey &&
            settings.host &&
            !settings.host.startsWith('/') && (
              <Alert severity="success" sx={{ mt: 2 }}>
                <Typography variant="body2">
                  <strong>HTTPS Access URL:</strong> https://{settings.host || 'localhost'}:
                  {settings.port || 8080}
                </Typography>
              </Alert>
            )}

          {(settings.sslCert && !settings.sslKey) ||
            (!settings.sslCert && settings.sslKey && (
